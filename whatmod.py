...
class WhatModQtApp(QMainWindow):
    ...
    def _build_hotkey_test_tab(self):
        ...
        title.setStyleSheet("font-size: 22px; font-weight: 800;")
        v.addWidget(title)
        info = QLabel("Use this to test whether a shortcut is likely to work globally. On macOS, bare keys like 1/2/3 are not reliable while Chrome is focused. Use Ctrl+Option+number for best results.")
    ...
    def add_custom_main_tab_dialog(self):
        ...
        if not ok or not name.strip():
            return
        key = self.unique_custom_main_key(name.strip())
        ...
        if key not in self.main_nav_order:
            try:
                idx = self.main_nav_order.index("Messages")
            except ValueError:
                idx = len(self.main_nav_order)
            self.main_nav_order.insert(idx, key)
        self.add_main_tab(QWidget(), key)
        ...
        self.set_status(f"Added {name.strip()}.")
    ...
    def edit_hotkey(self, seq: str):
        ...
        if dlg.exec() == QDialog.DialogCode.Accepted and dlg.sequence.strip():
            ...
            if new_seq != seq and new_seq in self.hotkeys:
                existing = self.hotkeys.get(new_seq, {})
