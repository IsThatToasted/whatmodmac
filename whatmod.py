...
WHATNOT_SELLER_TEST_URL = "https://www.whatnot.com/dashboard/live/"
...
DEFAULT_MAC_RELEASE_REPO = os.environ.get("WHATMOD_MAC_RELEASE_REPO", "IsThatToasted/whatnotmac")
...
# Common Whatnot/GitHub-style emoji shortcodes. Kept as plain text shortcodes
# instead of native emoji characters so they insert consistently on macOS,
# Windows, Linux, and inside Whatnot chat inputs.
WHATNOT_EMOJI_SHORTCODES = [
    ":smile:", ":smiley:", ":grinning:", ":grin:", ":laughing:", ":joy:", ":rofl:", ":blush:", ":wink:", ":relaxed:",
    ":heart_eyes:", ":kissing_heart:", ":star_struck:", ":sunglasses:", ":smirk:", ":yum:", ":stuck_out_tongue:", ":thinking:",
    ":neutral_face:", ":expressionless:", ":flushed:", ":sweat_smile:", ":sweat:", ":sob:", ":cry:", ":weary:", ":worried:",
    ":confused:", ":slightly_smiling_face:", ":upside_down_face:", ":innocent:", ":face_with_monocle:", ":partying_face:",
    ":heart:", ":orange_heart:", ":yellow_heart:", ":green_heart:", ":blue_heart:", ":purple_heart:", ":black_heart:",
    ":white_heart:", ":brown_heart:", ":two_hearts:", ":sparkling_heart:", ":heartpulse:", ":fire:", ":100:", ":boom:",
    ":sparkles:", ":star:", ":zap:", ":dash:", ":moneybag:", ":gem:", ":trophy:", ":medal_sports:", ":crown:",
    ":raised_hands:", ":clap:", ":pray:", ":muscle:", ":thumbsup:", ":thumbsdown:", ":ok_hand:", ":point_up:",
    ":point_down:", ":point_left:", ":point_right:", ":wave:", ":eyes:", ":shopping_cart:", ":gift:", ":package:",
    ":label:", ":money_with_wings:", ":dollar:", ":credit_card:", ":truck:", ":rocket:", ":alarm_clock:", ":hourglass:",
    ":rotating_light:", ":warning:", ":white_check_mark:", ":x:", ":heavy_check_mark:", ":heavy_multiplication_x:",
    ":exclamation:", ":question:", ":grey_exclamation:", ":grey_question:", ":bell:", ":mega:", ":loudspeaker:",
    ":runner:", ":athletic_shoe:", ":shirt:", ":jeans:", ":dress:", ":handbag:", ":purse:", ":watch:", ":ring:",
    ":lipstick:", ":briefcase:", ":camera:", ":video_camera:", ":tv:", ":game_die:", ":dart:", ":slot_machine:",
    ":soccer:", ":basketball:", ":football:", ":baseball:", ":tennis:", ":golf:", ":boxing_glove:", ":8ball:",
    ":dog:", ":cat:", ":bear:", ":panda_face:", ":monkey_face:", ":unicorn:", ":butterfly:", ":rose:",
    ":sunny:", ":cloud:", ":snowflake:", ":rainbow:", ":pizza:", ":hamburger:", ":fries:", ":taco:", ":coffee:",
]

SELLER_TEST_CHAT_INPUT_SELECTORS = [
    'input.chatInput[data-cy="chat_text_field"]',
    'input[data-cy="chat_text_field"]',
                ...
                if isinstance(incoming, list):
                    for i, item in enumerate(incoming, 1):
                        if isinstance(item, dict):
                            slots.append(MessageSlot(str(item.get("title", f"Message {i}")), str(item.get("body", "")), bool(item.get("announce", False))))
                        else:
                            slots.append(MessageSlot(f"Message {i}", str(item), False))
                tabs[tab] = slots
...
class WhatnotBrowser:
    ...
    def _worker_loop(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                if command == "launch":
                    seller_testing = bool(args[0]) if args else False
                    if alive():
                        self._queue_status("Browser already connected." + (" Seller Testing is on." if seller_testing else ""))
                    ...
                    self._queue_status("Launching Chrome" + (" for Seller Testing..." if seller_testing else "..."))
                    try:
                        ...
                        page.goto(WHATNOT_SELLER_TEST_URL if seller_testing else WHATNOT_URL, wait_until="domcontentloaded")
                        self._set_ready(True)
                        if seller_testing:
                            self._queue_status("Chrome opened in Seller Testing mode. Use your seller dashboard/live chat input, then click a message card.")
                        else:
                ...
                if command == "send":
                    ...
                    announce_sequence = bool(args[1]) if len(args) > 1 else False
                    seller_testing = bool(args[2]) if len(args) > 2 else False
                    if not message:
                    ...
                    chat_input = None
                    selectors = (SELLER_TEST_CHAT_INPUT_SELECTORS + CHAT_INPUT_SELECTORS) if seller_testing else CHAT_INPUT_SELECTORS
                    ...
                    if chat_input is None:
                        if seller_testing:
                            raise RuntimeError("Could not find the Seller Testing chat input. Open Whatnot dashboard/live and make sure the dark seller chat input is visible.")
                        raise RuntimeError("Could not find chat input. Open a live show with chat visible and try again.")
                    chat_input.click()
                    if announce_sequence:
                        # Whatnot announce mode is more reliable when the command, a space,
                        # and the body are inserted in order instead of as one prebuilt fill.
                        body = message.strip()
                        if body.lower().startswith("/announce"):
                            body = re.sub(r"^/announce\s*", "", body, flags=re.IGNORECASE).strip()
                        try:
                            chat_input.fill("")
                        except Exception:
                            pass
                        chat_input.click()
                        try:
                            page.keyboard.insert_text("/announce")
                            page.keyboard.press("Space")
                            page.keyboard.insert_text(body)
                        except Exception:
                            # Fallback for unusual input implementations. The final visible
                            # text is still correct even if sequential typing is unavailable.
                            chat_input.fill(add_announce_prefix(body))
                        shown = add_announce_prefix(body)
                    else:
                        chat_input.fill(message)
                        shown = message
                    chat_input.press("Enter")
                    self._queue_status(f"Sent: {shown[:90]}{'...' if len(shown) > 90 else ''}")
                    result_q.put((True, True))
                    continue
    ...
    def launch(self, seller_testing: bool = False) -> None:
        self._call_worker("launch", bool(seller_testing), timeout=60.0)
        self.drain_status_messages()
    ...
    def send_message(self, message: str, announce_sequence: bool = False, seller_testing: bool = False) -> None:
        self._call_worker("send", message, bool(announce_sequence), bool(seller_testing), timeout=12.0)
        self.drain_status_messages()
...
class AppSettingsStore:
    ...
    def load(cls) -> Dict[str, object]:
        ...
        if data["theme"] not in {"Default", "Pink"}:
            data["theme"] = "Default"
        data["seller_testing"] = bool(data.get("seller_testing", cls.DEFAULTS["seller_testing"]))
    ...
    def save(settings: Dict[str, object]) -> None:
        ...
        payload = {
            ...
            "seller_testing": bool(settings.get("seller_testing", False)),
...
class MessageCardWidget(QFrame):
    def __init__(self, title: str, body: str, send_cb, copy_cb, key_cb=None, edit_cb=None, remove_cb=None, drag_tab: str = "", drag_index: int = -1, drag_enabled: bool = False, announce_enabled: bool = False, announce_cb=None):
        super().__init__()
        self._body = body
        ...
        self._announce_enabled = bool(announce_enabled)
        ...
        if self._announce_cb is not None:
            ...
            announce_box = QCheckBox("/announce this card")
            announce_box.setChecked(self._announce_enabled)
            announce_box.setToolTip("Uses /announce for this card even when the global /Announce setting is off.")
            announce_box.toggled.connect(lambda checked: self._announce_cb(bool(checked)))
            announce_row.addWidget(announce_box)
...
class WhatModQtApp(QMainWindow):
    def __init__(self):
        ...
        self.seller_testing = bool(self.app_settings.get("seller_testing", False))
        ...
        self.message_editors: Dict[str, List[tuple]] = {}
    ...
    def _build_general_settings_tab(self):
        ...
        self.announce_box.setToolTip("Global override: when on, every outgoing message sends as /announce.")
        self.announce_box.toggled.connect(self.set_announce_mode)
        general_layout.addWidget(self.announce_box)

        announce_hint = QLabel("Global override: when on, every outgoing message sends as /announce. When off, only cards marked /announce this card announce.")
        announce_hint.setObjectName("sectionHint")
        announce_hint.setWordWrap(True)
        general_layout.addWidget(announce_hint)

        self.seller_testing_box = QCheckBox("Seller Testing")
        self.seller_testing_box.setChecked(bool(getattr(self, "seller_testing", False)))
        self.seller_testing_box.setToolTip("Targets the seller dashboard/live chat input, including the dark chatInput field used when you are the seller.")
        self.seller_testing_box.toggled.connect(self.set_seller_testing)
        general_layout.addWidget(self.seller_testing_box)

        seller_testing_hint = QLabel("Use this when testing from your own seller dashboard. Launch Browser will open dashboard/live, and sends will target the dark seller chat input: data-cy=chat_text_field.")
        seller_testing_hint.setObjectName("sectionHint")
        seller_testing_hint.setWordWrap(True)
        general_layout.addWidget(seller_testing_hint)
    ...
    def import_custom_dash_card(self, tab: Optional[str] = None):
        ...
        if not isinstance(data, dict):
            ...
            return
        self.tabs_data.setdefault(tab, []).append(MessageSlot(str(data.get("title", "Imported Card")), str(data.get("body", "")), bool(data.get("announce", False))))
        self._render_custom_dash_tab(tab)
    ...
    def send_custom_dash_message(self, message: str, tab: Optional[str] = None):
        # Backward-compatible wrapper. Per-card /announce is handled by send_card_message.
        self.send_message(message, force_announce=False)

    def copy_custom_dash_message(self, message: str, tab: Optional[str] = None):
        # Backward-compatible wrapper. Per-card /announce is handled by copy_card_message.
        self.copy_to_clipboard(message, apply_announce=True, force_announce=False)

    def card_announce_enabled(self, tab: str, index: int) -> bool:
        arr = self.tabs_data.get(str(tab), [])
        return bool(0 <= int(index) < len(arr) and getattr(arr[int(index)], "announce", False))

    def set_card_announce(self, tab: str, index: int, checked: bool) -> None:
        arr = self.tabs_data.get(str(tab), [])
        if 0 <= int(index) < len(arr):
            arr[int(index)].announce = bool(checked)
            self.save_all(silent=True)
            state = "enabled" if checked else "disabled"
            self.set_status(f"/announce {state} for {arr[int(index)].title or 'this card'}.")

    def send_card_message(self, tab: str, index: int, message: Optional[str] = None):
        arr = self.tabs_data.get(str(tab), [])
        if 0 <= int(index) < len(arr):
            slot = arr[int(index)]
            self.send_message(slot.body if message is None else message, force_announce=bool(getattr(slot, "announce", False)))
        else:
            self.send_message(message or "", force_announce=False)

    def copy_card_message(self, tab: str, index: int, message: Optional[str] = None):
        arr = self.tabs_data.get(str(tab), [])
        if 0 <= int(index) < len(arr):
            slot = arr[int(index)]
            self.copy_to_clipboard(slot.body if message is None else message, apply_announce=True, force_announce=bool(getattr(slot, "announce", False)))
        else:
            self.copy_to_clipboard(message or "", apply_announce=True, force_announce=False)
    ...
    def mini_control_items_for_tab(self, tab: Optional[str] = None) -> List[tuple[str, str, str]]:
        ...
        if tab == "Shoes":
            ...
            for ix, slot in enumerate(self.tabs_data.get(str(tab), [])):
                title = (slot.title or "Untitled").strip()
                body = (slot.body or "").strip()
                if body:
                    items.append((title, body, f"{str(tab)}|{ix}"))
        return items
    ...
    def open_mini_controls(self, tab: Optional[str] = None) -> None:
        ...
        for i, (title, body, source_tab) in enumerate(items):
            ...
            if "|" in str(source_tab):
                tab_key, index_text = str(source_tab).rsplit("|", 1)
                try:
                    card_index = int(index_text)
                except Exception:
                    card_index = -1
                send.clicked.connect(lambda _=False, b=body, t=tab_key, ix=card_index: self.send_card_message(t, ix, b))
                copy.clicked.connect(lambda _=False, b=body, t=tab_key, ix=card_index: self.copy_card_message(t, ix, b))
            else:
    ...
    def _render_custom_dash_tab(self, tab: str = "Commands"):
        ...
        top.addWidget(add_btn, 0, 2)
        top.addWidget(clear_btn, 0, 3)
        top.addWidget(mini_btn, 0, 4)
        ...
        if self.view_mode == "Edit":
            ...
            for i, slot in enumerate(arr, 1):
                ...
                announce_box = QCheckBox("/announce this card")
                announce_box.setChecked(bool(getattr(slot, "announce", False)))
                grid.addWidget(announce_box, 2, 1, 1, 2)
                actions = [
                    ("Send", lambda _=False, b=body_edit, ix=i-1, t=tab: self.send_card_message(t, ix, b.toPlainText()), "primaryButton"),
                    ("Copy", lambda _=False, b=body_edit, ix=i-1, t=tab: self.copy_card_message(t, ix, b.toPlainText()), "ghostButton"),
                    ("Emojis", lambda _=False, b=body_edit: self.show_emoji_picker(b), "ghostButton"),
                ...
                layout.addWidget(box)
                self.message_editors[tab].append((title_edit, body_edit, announce_box))
        else:
            ...
            for n, (i, slot) in enumerate(items):
                card = MessageCardWidget(slot.title or f"Custom Card {i+1}", slot.body, lambda m, ix=i, t=tab: self.send_card_message(t, ix, m), lambda m, ix=i, t=tab: self.copy_card_message(t, ix, m), lambda ix=i,t=tab: self.assign_hotkey_dialog(t, ix), lambda ix=i,t=tab: self.edit_message_card_dialog(t, ix), lambda ix=i,t=tab: self.remove_message_card(t, ix), drag_tab=tab, drag_index=i, drag_enabled=allow_reorder, announce_enabled=bool(getattr(slot, "announce", False)), announce_cb=lambda checked, ix=i, t=tab: self.set_card_announce(t, ix, checked))
    ...
    def _render_message_tab(self, tab_name: str):
        ...
        if self.view_mode == "Edit":
            ...
            for i, slot in enumerate(arr, 1):
                ...
                grid.addWidget(announce_box, 2, 1, 1, 2)
                actions = [
                    ("Send", lambda _=False, b=body, ix=i-1, t=tab_name: self.send_card_message(t, ix, b.toPlainText()), ""),
                    ("Copy", lambda _=False, b=body, ix=i-1, t=tab_name: self.copy_card_message(t, ix, b.toPlainText()), "ghostButton"),
                    ("Emojis", lambda _=False, b=body: self.show_emoji_picker(b), "ghostButton"),
                ...
                for col, (txt, cb, obj) in enumerate(actions):
                    ...
                    btn.clicked.connect(cb)
                    grid.addWidget(btn, 3, col)
                layout.addWidget(box); self.message_editors[tab_name].append((title, body, announce_box))
        else:
            allow_reorder = not bool(q)
            ...
            for n, (i, slot) in enumerate(items):
                card = MessageCardWidget(slot.title or f"Message {i+1}", slot.body, lambda m, ix=i, t=tab_name: self.send_card_message(t, ix, m), lambda m, ix=i, t=tab_name: self.copy_card_message(t, ix, m), lambda ix=i,t=tab_name: self.assign_hotkey_dialog(t, ix), lambda ix=i,t=tab_name: self.edit_message_card_dialog(t, ix), lambda ix=i,t=tab_name: self.remove_message_card(t, ix), drag_tab=tab_name, drag_index=i, drag_enabled=allow_reorder, announce_enabled=bool(getattr(slot, "announce", False)), announce_cb=lambda checked, ix=i, t=tab_name: self.set_card_announce(t, ix, checked))
                gridw.add_card(card, n//3, n%3)
            gridw.grid.setRowStretch((len(items)+2)//3, 1)
            if q:
            ...
            if self.view_mode == "Edit" and key not in {"All Shoes", "Youth Sizes"}:
                ...
                for i, d in items:
                    ...
                    for col, (txt, cb, obj) in enumerate(actions):
                        ...
                        if obj:
                            btn.setObjectName(obj)
                        btn.clicked.connect(cb); grid.addWidget(btn,3,col)
                    layout.addWidget(box)
            else:
    ...
    def insert_emoji_shortcode(self, editor: QTextEdit, shortcode: str) -> None:
        """Insert an emoji shortcode into a QTextEdit in a Qt/macOS-safe way."""
        if editor is None:
            return
        ...
        try:
            ...
            after = editor.toPlainText()[after_pos:after_pos + 1]
            prefix = "" if not before or before.endswith((" ", "\n", "\t")) else " "
            suffix = "" if not after or after in {" ", "\n", "\t"} else " "
            cursor.insertText(f"{prefix}{code}{suffix}")
            editor.setTextCursor(cursor)
            editor.setFocus(Qt.FocusReason.OtherFocusReason)
        except Exception:
            try:
                editor.insertPlainText(f" {code} ")
            except Exception:
                pass

    def show_emoji_picker(self, editor: QTextEdit) -> None:
        """Show a simple shortcode picker that works the same on macOS/Windows/Linux."""
        dlg = QDialog(self)
        dlg.setWindowTitle("Whatnot Emojis")
        dlg.resize(720, 520)
        root = QVBoxLayout(dlg)
        title = QLabel("Click an emoji shortcode to add it to this message.")
        title.setObjectName("sectionTitle")
        root.addWidget(title)
        hint = QLabel("Shortcodes insert as text like :smile: or :heart:, which is safer than using the OS emoji keyboard on macOS builds.")
        hint.setObjectName("sectionHint")
        hint.setWordWrap(True)
        root.addWidget(hint)

        search = QLineEdit()
        search.setPlaceholderText("Search emojis, for example heart, fire, smile, shoe…")
        ...
        def render(filter_text: str = "") -> None:
            while grid.count():
                ...
                if item.widget():
                    item.widget().deleteLater()
            q = str(filter_text or "").strip().lower().replace(":", "")
            matches = [c for c in WHATNOT_EMOJI_SHORTCODES if not q or q in c.lower().replace(":", "")]
            for n, shortcode in enumerate(matches):
                btn = QPushButton(shortcode)
                btn.setObjectName("ghostButton")
                btn.setToolTip(f"Insert {shortcode}")
                btn.clicked.connect(lambda _=False, code=shortcode: self.insert_emoji_shortcode(editor, code))
                grid.addWidget(btn, n // 4, n % 4)
            if not matches:
                none = QLabel("No matching shortcodes.")
                none.setObjectName("sectionHint")
                grid.addWidget(none, 0, 0)
            grid.setRowStretch((len(matches) + 3) // 4, 1)
    ...
    def edit_message_card_dialog(self, tab: str, index: int):
        ...
        emoji_btn.setObjectName("ghostButton")
        emoji_btn.clicked.connect(lambda _=False, ed=body: self.show_emoji_picker(ed))
        v.addWidget(emoji_btn)
        announce_box = QCheckBox("/announce this card")
        announce_box.setChecked(bool(getattr(slot, "announce", False)))
        v.addWidget(announce_box)
        row = QHBoxLayout()
        ...
        v.addLayout(row)
        send_btn.clicked.connect(lambda: self.send_message(body.toPlainText(), force_announce=announce_box.isChecked()))
        copy_btn.clicked.connect(lambda: self.copy_to_clipboard(body.toPlainText(), force_announce=announce_box.isChecked()))
        cancel_btn.clicked.connect(dlg.reject)
        def save_card():
            arr[index] = MessageSlot(title.text().strip() or f"Message {index+1}", body.toPlainText().strip(), bool(announce_box.isChecked()))
            self.save_all(silent=True)
            self.refresh_current_tab()
            dlg.accept()
    ...
    def handle_hotkey(self, payload: Dict[str, object]):
        tab = str(payload.get('tab',''))
        index = int(payload.get('index',0))
        msg = self.resolve_hotkey_message(tab, index)
        if msg:
            self.send_message(msg, force_announce=self.card_announce_enabled(tab, index))
    ...
    def collect_tab(self, tab_name: str):
        ...
        if editors:
            ...
            for item in editors:
                title_edit = item[0]
                body_edit = item[1]
                announce_box = item[2] if len(item) > 2 else None
                rebuilt.append(MessageSlot(title_edit.text().strip(), body_edit.toPlainText().strip(), bool(announce_box.isChecked()) if announce_box is not None else False))
            self.tabs_data[tab_name] = rebuilt
    ...
    def save_app_settings(self):
        ...
        self.app_settings["seller_testing"] = bool(getattr(self, "seller_testing", False))
        self.app_settings["experimental_bare_hotkeys"] = bool(getattr(self, "experimental_bare_hotkeys", False))
        AppSettingsStore.save(self.app_settings)

    def set_seller_testing(self, checked: bool):
        self.seller_testing = bool(checked)
        self.save_app_settings()
        self.set_status("Seller Testing enabled. Launch Browser opens dashboard/live and sends target the seller chat input." if self.seller_testing else "Seller Testing disabled. Sends target normal live chat.")
    ...
    def set_announce_mode(self, checked: bool):
        ...
        self.save_app_settings()
        self.set_status("/Announce global enabled. All outgoing messages will announce." if self.announce_mode else "/Announce global disabled. Only cards marked /announce this card will announce.")
    ...
    def _serialize_state_for_snapshot(self) -> Dict[str, object]:
        return {
            ...
            "app_settings": {
                ...
                "seller_testing": bool(getattr(self, "seller_testing", False)),
            },
        }
    ...
    def duplicate_message(self, tab: str, index: int):
        ...
        if 0 <= index < len(arr):
            source = arr[index]
            arr.insert(index + 1, MessageSlot((source.title or f"Message {index + 1}") + " Copy", source.body, bool(getattr(source, "announce", False))))
            self.refresh_current_tab()
            self.set_status("Duplicated message card.")

    def should_send_as_announce(self, force_announce: bool = False) -> bool:
        """Return True when a message should use Whatnot /announce.

        The Settings /Announce toggle is a global override for all outgoing
        messages. A card-level /announce toggle also works independently for
        just that card. This makes testing simpler: either toggle can cause the
        outgoing message to be announced.
        """
        return bool(getattr(self, "announce_mode", False) or force_announce)

    def format_outgoing_message(self, message: str, force_announce: bool = False) -> str:
        msg = (message or "").strip()
        if not msg:
            return msg
        return add_announce_prefix(msg) if self.should_send_as_announce(force_announce) else msg

    def send_message(self, message: str, force_announce: bool = False):
        raw_msg = (message or "").strip()
        msg = self.format_outgoing_message(raw_msg, force_announce=force_announce)
        if not msg: return
        announce_sequence = self.should_send_as_announce(force_announce)
        if not self.enter_to_send:
            self.copy_to_clipboard(raw_msg, apply_announce=True, force_announce=force_announce); return
        try:
            # Pass the raw message and let the browser worker enter /announce,
            # then a space, then the message in sequence. This avoids Whatnot
            # ignoring a prebuilt pasted string in some chat layouts.
            self.browser.send_message(raw_msg if announce_sequence else msg, announce_sequence=announce_sequence, seller_testing=bool(getattr(self, "seller_testing", False)))
        except Exception as e:
            self.copy_to_clipboard(raw_msg, apply_announce=True, force_announce=force_announce); QMessageBox.warning(self, "Browser not ready", f"Copied instead.\n\n{e}")

    def copy_to_clipboard(self, text: str, apply_announce: bool=True, force_announce: bool = False):
        msg=self.format_outgoing_message(text, force_announce=force_announce) if apply_announce else text.strip()
        if copy_text(msg): self.set_status(f"Copied: {msg[:90]}")

    def launch_browser(self):
        try:
            self.browser.launch(seller_testing=bool(getattr(self, "seller_testing", False)))
    ...
    def _load_update_settings(self) -> Dict[str, object]:
        try:
            if UPDATE_FILE.exists():
                raw = json.loads(UPDATE_FILE.read_text(encoding="utf-8"))
                return raw if isinstance(raw, dict) else {}
        except Exception:
            pass
        return {}

    def _version_from_release_text(self, text: str) -> tuple[int, ...]:
        """Extract a comparable version from a GitHub release title/tag."""
        found = re.findall(r"\d+(?:\.\d+){0,3}", str(text or ""))
        return parse_version_tuple(found[0]) if found else (0,)

    def _github_latest_release(self, repo: str) -> Dict[str, object]:
        repo = str(repo or "").strip().strip("/")
        if not re.fullmatch(r"[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+", repo):
            raise ValueError("Mac release repo must look like owner/repo, for example IsThatToasted/whatnotmac.")
        url = f"https://api.github.com/repos/{repo}/releases/latest"
        request = urllib.request.Request(
            url,
            headers={
                "Accept": "application/vnd.github+json",
                "User-Agent": f"WhatMod/{APP_VERSION}",
        ...
        if not isinstance(data, dict):
            raise ValueError("GitHub did not return a valid release response.")
        return data

    def _choose_mac_release_asset(self, release: Dict[str, object]) -> Optional[Dict[str, object]]:
        ...
        for asset in assets:
            ...
            if "mac" in lower or "darwin" in lower or "osx" in lower:
                score += 10
            if platform.machine().lower() in lower:
                score += 4
    ...
    def _download_file_with_progress(self, url: str, target: Path, title: str = "Downloading Update") -> Path:
        target.parent.mkdir(parents=True, exist_ok=True)
        request = urllib.request.Request(url, headers={"User-Agent": f"WhatMod/{APP_VERSION}"})
        with urllib.request.urlopen(request, timeout=30) as resp:
            total = int(resp.headers.get("Content-Length", "0") or 0)
            progress = QProgressDialog(title, "Cancel", 0, total if total > 0 else 0, self)
    ...
    def check_mac_release_update(self) -> None:
        settings = self._load_update_settings()
        repo = str(settings.get("mac_release_repo") or os.environ.get("WHATMOD_MAC_RELEASE_REPO") or DEFAULT_MAC_RELEASE_REPO).strip()
        release = self._github_latest_release(repo)
        title = str(release.get("name") or release.get("tag_name") or "Latest release")
        tag = str(release.get("tag_name") or "")
        release_version = max(self._version_from_release_text(title), self._version_from_release_text(tag))
        current_version = parse_version_tuple(APP_VERSION)
        if release_version and release_version > current_version:
            asset = self._choose_mac_release_asset(release)
            if not asset:
                html_url = str(release.get("html_url", "") or "")
                QMessageBox.information(self, "Update Available", f"{title} is available.\n\nNo .dmg/.pkg/.zip asset was found, so the release page will open.")
                if html_url:
                    webbrowser.open(html_url)
                return
            asset_name = str(asset.get("name") or "WhatMod-Mac-Update")
            download_url = str(asset.get("browser_download_url") or "")
            answer = QMessageBox.question(
                self,
                "Mac Update Available",
                f"{title} is available.\n\nDownload and open {asset_name}?\n\nThe app cannot safely replace itself while running, so macOS will open the downloaded update for install.",
            )
            ...
            self._download_file_with_progress(download_url, target, f"Downloading {asset_name}")
            if sys.platform == "darwin":
                subprocess.Popen(["open", str(target)])
            else:
                webbrowser.open(target.as_uri())
            QMessageBox.information(self, "Update Downloaded", f"Downloaded update to:\n{target}\n\nOpen the downloaded file and replace the old app when prompted.")
        else:
            QMessageBox.information(self, "Updates", f"You are up to date.\n\nLatest Mac release checked: {title}")

    def check_windows_update_download(self) -> None:
        """Download the latest Windows build instead of only displaying a URL.

        Windows can run the downloaded updater/exe, but the currently running
        executable should not try to overwrite itself. The safest cross-build
        behavior is to download the new WhatMod.exe to Downloads and open it.
        """
        settings = self._load_update_settings()
        download_url = str(
            settings.get("windows_download_url")
            or os.environ.get("WHATMOD_WINDOWS_UPDATE_URL")
            or DEFAULT_WINDOWS_UPDATE_URL
        ).strip()
        latest_version = "latest"
        should_download = True

        # Keep backward compatibility with the existing manifest/version check.
        # If the manifest is unavailable, we still allow downloading the fixed
        # GitHub release asset because that is now the requested Windows update path.
        try:
            manifest_url = normalize_update_manifest_url(str(settings.get('manifest_url', DEFAULT_UPDATE_MANIFEST_URL)))
            manifest = self._fetch_json_url(manifest_url)
            ...
            if isinstance(latest, dict):
                latest_version = str(latest.get('version', 'latest') or 'latest')
                manifest_download = str(latest.get('download_url', '') or '').strip()
                if manifest_download:
                    download_url = manifest_download
                should_download = parse_version_tuple(latest_version) > parse_version_tuple(APP_VERSION)
        except Exception:
            latest_version = "latest"
            should_download = True
        ...
        if not should_download:
            answer = QMessageBox.question(
                self,
                "Updates",
                "You appear to be up to date.\n\nDownload the latest Windows build anyway?",
            )
            if answer != QMessageBox.Yes:
                return
        else:
            answer = QMessageBox.question(
                ...
                f"Version {latest_version} is available.\n\nDownload and open the latest WhatMod.exe?",
            )
            if answer != QMessageBox.Yes:
                return

        target_dir = Path.home() / "Downloads"
        target = target_dir / "WhatMod.exe"
        if target.exists():
            target = target_dir / f"WhatMod-{int(time.time())}.exe"
        self._download_file_with_progress(download_url, target, "Downloading WhatMod.exe")
        try:
        ...
        QMessageBox.information(
            self,
            "Update Downloaded",
            f"Downloaded update to:\n{target}\n\nClose the old WhatMod window after the new version opens.",
        )

    def check_for_updates(self):
        try:
            if sys.platform == "darwin":
                self.check_mac_release_update()
                return
            if sys.platform.startswith("win"):
                self.check_windows_update_download()
                return
            settings = self._load_update_settings()
            ...
            if parse_version_tuple(new) > parse_version_tuple(APP_VERSION):
                ...
                if download_url:
                    answer = QMessageBox.question(self,"Update Available", f"Version {new} is available.\n\nDownload and open the update?")
                    if answer == QMessageBox.Yes:
                        target = Path.home() / "Downloads" / Path(urllib.parse.urlparse(download_url).path).name
                        self._download_file_with_progress(download_url, target, f"Downloading {target.name}")
                        webbrowser.open(target.as_uri())
                else:
                    QMessageBox.information(self,"Update Available", f"Version {new} is available, but no download URL was provided.")
...
def prepare_macos_qt_environment() -> None:
    """Apply safe macOS-only Qt defaults before QApplication is created.

    These settings are ignored on Windows/Linux because this function is only
    called on Darwin. They help PyInstaller .app launches become a normal
    foreground GUI app instead of sitting in the Dock/tray bounce state.
    """
    if not is_macos():
        return
...
def activate_macos_foreground(app: QApplication, window: Optional[QMainWindow] = None) -> None:
    """Best-effort foreground activation for frozen macOS app bundles.

    PyInstaller/Qt apps can occasionally launch but never get focus on macOS.
    PyObjC is optional; if it is not bundled, the osascript fallback still works
    on standard macOS installs. All failures are swallowed so the app remains
    cross-platform and does not crash if an activation method is unavailable.
    """
    if not is_macos():
        return
...
def show_window_robustly(app: QApplication, window: QMainWindow) -> None:
    """Show and re-activate the main window a few times for macOS launch edge cases."""
    window.show()
    window.raise_()
    window.activateWindow()
    if is_macos():
        activate_macos_foreground(app, window)
        for delay in (150, 500, 1200):
            QTimer.singleShot(delay, lambda w=window, a=app: activate_macos_foreground(a, w))


def main() -> None:
    prepare_macos_qt_environment()

    app = QApplication(sys.argv)
    app.setApplicationName(APP_NAME)
    app.setApplicationDisplayName(APP_NAME)
    app.setOrganizationName("WhatMod")
    ...
    # On macOS, show the real main window immediately. This avoids a common
    # frozen-app edge case where the Dock icon bounces while a transient splash
    # window is shown/closed before the main window becomes active. Windows and
    # Linux keep the startup splash behavior.
    if is_macos():
        win = WhatModQtApp()
        show_window_robustly(app, win)
        sys.exit(app.exec())
