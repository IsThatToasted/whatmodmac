...
QListWidget#messageNavList::item {
    background: transparent;
    border-radius: 9px;
    padding: 6px 8px;
    min-height: 24px;
}
QListWidget#messageNavList::item:hover {
    background: #FFE9F4;
    color: #831843;
}
QListWidget#messageNavList::item:selected {
    background: #FFFFFF;
    color: #831843;
    border: 1px solid #E9559D;
}
...
class GlobalHotkeyManager:
    """Optional global hotkey bridge using pynput. Falls back to in-app QShortcut only."""
    ...
    @staticmethod
    def _convert(seq: str) -> str:
        parts = [p.strip() for p in re.split(r"\+", seq or "") if p.strip()]
        out = []
        for part in parts:
            low = part.lower().replace("option", "alt").replace("cmd", "cmd").replace("command", "cmd").replace("meta", "cmd")
    ...
    def start(self, hotkeys: Dict[str, Dict[str, object]]) -> None:
        self.stop()
        try:
        ...
        for seq, payload in hotkeys.items():
            try:
                converted = self._convert(seq)
                if converted:
                    mapping[converted] = (lambda p=dict(payload): self.callback(p))
            except Exception:
                continue
        if not mapping:
            self.error = "No global hotkeys assigned."
            return
        try:
...
class WhatModQtApp(QMainWindow):
    ...
    def _rebuild_sidebar_buttons(self) -> None:
        ...
        if getattr(self, "messages_nav_expanded", False):
            ...
            message_keys = [t for t in MESSAGE_TABS if t != "Commands"]
            for key in message_keys:
                self.message_nav_list.addItem(self.tab_label(key))
                self.message_nav_list.item(self.message_nav_list.count() - 1).setData(Qt.ItemDataRole.UserRole, key)
    ...
    def apply_sidebar_message_order_from_drag(self) -> None:
        ...
        for i in range(nav.count()):
            ...
            if key and key not in dragged_order and key != "Commands":
    ...
    def _build_settings_tab(self):
        ...
        announce_hint.setStyleSheet("color: #9D4B73;")
    ...
    def _render_custom_dash_tab(self):
        ...
        if self.view_mode == "Edit":
            ...
            gridw = MessageCardGrid(self, "Commands", allow_reorder=allow_reorder)
            ...
            for n, (i, slot) in enumerate(items):
                card = MessageCardWidget(slot.title or f"Custom Card {i+1}", slot.body, self.send_custom_dash_message, self.copy_custom_dash_message, lambda ix=i: self.assign_hotkey_dialog("Commands", ix), lambda ix=i: self.edit_message_card_dialog("Commands", ix), lambda ix=i: self.remove_message_card("Commands", ix), drag_tab="Commands", drag_index=i, drag_enabled=allow_reorder)
    ...
    def _render_message_tab(self, tab_name: str):
        ...
        hint.setObjectName("sectionHint")
        hint.setWordWrap(True)
        top_layout.addWidget(hint, 1, 0, 1, 7)
        layout.addWidget(top_card)
    ...
    def register_hotkeys(self):
        ...
        for seq,payload in self.hotkeys.items():
            try:
                ...
                pass
        try:
            self.global_hotkey_manager.start(self.hotkeys)
